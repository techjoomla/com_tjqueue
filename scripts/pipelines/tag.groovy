...
pipeline {
    ...
    stages {
        ...
        stage('Init') {
            steps {
                script {
                    ...
                    // Get tag name
                    def  tjqueueVersion = env.getProperty("tjqueueVersion")
                    echo tjqueueVersion
        ...
        stage('Copy files') {
            steps {
                script {
                    ...
                    sh("rsync -a dbal/ builds/com_tjqueue/media/libs/")
                    sh("rsync -a sqs/ builds/com_tjqueue/media/libs/")
                    sh("mv builds/com_tjqueue/media/libs/composer_files/* builds/com_tjqueue/media/libs/")
                    sh("rm -rf builds/com_tjqueue/media/libs/composer_files")
                }
            }
        }
       ...
       stage('composer') {
           steps {
               script {
                   sh("cd builds/com_tjqueue/media/libs/ && composer install --no-dev --ignore-platform-reqs")
                    def cleanUpList = [".git","tests","docs","Tests"] as String[]
                    def deleteCmd = '-exec rm -r "{}" +';
                    for (item in cleanUpList) {
                       sh("cd builds/com_tjqueue/media/libs/ && find . -name "+item+" "+ deleteCmd);
                    }
                    ...
                    for (item in awsDataClenaup) {
                       sh("cd builds/com_tjqueue/media/libs/vendor/aws/aws-sdk-php/src/data && rm -rf "+item);
                    }
               }
           }
       }
