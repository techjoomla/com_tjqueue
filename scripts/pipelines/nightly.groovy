...
pipeline {
    ...
    stages {
        ...
        stage('Copy files') {
            steps {
                script {
                    ...
                    sh("rsync -a dbal/ builds/com_tjqueue/admin/libraries/libs/")
                    sh("rsync -a sqs/ builds/com_tjqueue/admin/libraries/libs/")
                    sh("mv builds/com_tjqueue/admin/libraries/libs/composer_files/* builds/com_tjqueue/admin/libraries/libs/")
                    sh("rm -rf builds/com_tjqueue/admin/libraries/libs/composer_files")
                }
            }
        }
       ...
       stage('composer') {
           steps {
               script {
                   sh("cd builds/com_tjqueue/admin/libraries/libs/ && composer install --no-dev --ignore-platform-reqs")
                    def cleanUpList = [".git","tests","docs","Tests"] as String[]
                    def deleteCmd = '-exec rm -r "{}" +';
                    for (item in cleanUpList) {
                       sh("cd builds/com_tjqueue/admin/libraries/libs/ && find . -name "+item+" "+ deleteCmd);
                    }
                    ...
                    for (item in awsCleanUp) {
                       sh("cd builds/com_tjqueue/admin/libraries/libs/vendor/aws/aws-sdk-php/src && rm -rf "+item);
                    }
                    ...
                    for (item in awsDataClenaup) {
                       sh("cd builds/com_tjqueue/admin/libraries/libs/vendor/aws/aws-sdk-php/src/data && rm -rf "+item);
                    }
               }
           }
       }
